...
void validate(const RectangleGeometry &geometry, const MeshGenerationOptions &options)
{
    ...
    {
        throw std::invalid_argument("Rectangle length must be positive.");
    }
    ...
    {
        throw std::invalid_argument("Only triangular meshes are implemented for now.");
    }
}

RectangleGmshTags create_rectangle(const RectangleGeometry &geometry, double mesh_size)
{
    ...
    return {surface, inlet_group, wall_group, outlet_group};
}

Index local_node_index(const std::unordered_map<std::size_t, Index> &gmsh_to_local, std::size_t gmsh_node_tag)
{
...
std::unordered_map<std::size_t, Index> extract_nodes(RawMeshData &raw_mesh)
{
    ...
    for (Index i_node = 0; i_node < node_tags.size(); ++i_node)
    {
        ...
        raw_mesh.nodes.push_back({coordinates[3 * i_node], coordinates[3 * i_node + 1]});
    }

    return gmsh_to_local;
}

void extract_triangles(RawMeshData &raw_mesh, const std::unordered_map<std::size_t, Index> &gmsh_to_local,
                       int surface_tag)
{
    ...
    for (std::size_t i_type = 0; i_type < element_types.size(); ++i_type)
    {
        if (element_types[i_type] != gmsh_triangle_3_nodes)
        {
            throw std::runtime_error("Gmsh generated a non-triangular 2D element.");
        }
        ...
        if (nodes.size() % 3 != 0)
        {
            throw std::runtime_error("Invalid triangle connectivity returned by Gmsh.");
        }

        for (std::size_t i = 0; i < nodes.size(); i += 3)
        {
            raw_mesh.cell_types.push_back(CellType::Triangle);
            raw_mesh.cell_nodes.push_back(local_node_index(gmsh_to_local, nodes[i]));
            raw_mesh.cell_nodes.push_back(local_node_index(gmsh_to_local, nodes[i + 1]));
            raw_mesh.cell_nodes.push_back(local_node_index(gmsh_to_local, nodes[i + 2]));
...
void extract_boundary_edges(RawMeshData &raw_mesh, const std::unordered_map<std::size_t, Index> &gmsh_to_local,
                            int physical_group_tag, BoundaryId boundary_id)
{
        ...
        for (std::size_t i_type = 0; i_type < element_types.size(); ++i_type)
        {
            ...
            for (std::size_t i = 0; i < nodes.size(); i += 2)
            {
                raw_mesh.boundary_edges.push_back(
                    {{local_node_index(gmsh_to_local, nodes[i]), local_node_index(gmsh_to_local, nodes[i + 1])},
                     boundary_id});
            }
        }
    }
...
RawMeshData generate_mesh(const RectangleGeometry &geometry, const MeshGenerationOptions &options)
{
    ...
    raw_mesh.boundary_groups = {
        {inlet_boundary_id, "inlet"}, {wall_boundary_id, "wall"}, {outlet_boundary_id, "outlet"}};

    const auto gmsh_to_local{extract_nodes(raw_mesh)};

    extract_triangles(raw_mesh, gmsh_to_local, tags.surface);
