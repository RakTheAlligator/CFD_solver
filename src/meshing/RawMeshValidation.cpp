...
void throw_validation_error(const std::string& message)
{
    throw std::runtime_error(
        "Raw mesh validation failed: " + message);
}
...
void validate_nodes(const RawMeshData& raw_mesh)
{
    ...
    for (Index node_id = 0; node_id < raw_mesh.nodes.size(); ++node_id)
    {
        ...
        {
            throw_validation_error(
                "node " + std::to_string(node_id) + " contains a non-finite coordinate.");
        }
    }
}
...
void validate_cell_storage(const RawMeshData& raw_mesh)
{
    ...
    {
        throw_validation_error(
            "cell_types array is empty.");
    }
    if (raw_mesh.cell_nodes.empty())
    {
        throw_validation_error(
            "cell_nodes array is empty.");
    }
    if (raw_mesh.cell_node_offsets.empty())
    {
        throw_validation_error(
            "cell_node_offsets array is empty.");
    }
    if (raw_mesh.cell_node_offsets.size() != raw_mesh.cell_types.size() + 1)
    {
        throw_validation_error(
            "cell_node_offsets size must equal the number of cells + 1.");
    }
    if (raw_mesh.cell_node_offsets.front() != 0)
    {
        throw_validation_error(
            "cell_node_offsets must start at 0.");
    }
    ...
    for (std::size_t i = 1; i < raw_mesh.cell_node_offsets.size(); ++i)
    {
        ...
        {
            throw_validation_error(
                "cell_node_offsets must be strictly increasing.");
        }
        if (current > raw_mesh.cell_nodes.size())
        {
            throw_validation_error(
                "cell_node_offsets contains an offset outside cell_nodes.");
        }
    }
    ...
    {
        throw_validation_error(
            "last cell_node_offset must equal cell_nodes.size().");
    }
}

[[nodiscard]]
Index expected_node_count(
    ...
    const Index cell_id)
{
    ...
    throw_validation_error(
        "cell " + std::to_string(cell_id) + " has an unsupported CellType.");
}
...
void validate_cells(const RawMeshData& raw_mesh)
{
    for (Index cell_id = 0; cell_id < raw_mesh.cell_types.size(); ++cell_id)
    {
        ...
        {
            throw_validation_error(
                "cell " + std::to_string(cell_id) + " has " + std::to_string(node_count)
        ...
        for (Index position = begin; position < end; ++position)
        {
            ...
            {
                throw_validation_error(
                    "cell " + std::to_string(cell_id)+ " references node "
            ...
            for (Index previous_position = begin; previous_position < position; ++previous_position)
            {
                ...
                {
                    throw_validation_error(
                        "cell " + std::to_string(cell_id) + " contains duplicate node indices.");
                }
            }
        }
    }
...
[[nodiscard]]
BoundaryIdSet validate_boundary_groups(
    const RawMeshData& raw_mesh)
{
    if (raw_mesh.boundary_groups.empty())
    {
        throw_validation_error(
            "boundary_groups array is empty.");
    }
    ...
    for (const BoundaryGroup& group : raw_mesh.boundary_groups)
    {
        ...
        {
            throw_validation_error(
                "a boundary group has an empty name.");
        }
        ...
        {
            throw_validation_error(
                "boundary group ID " + std::to_string(group.id) + " is duplicated.");
        }
        ...
        {
            throw_validation_error(
                "boundary group name \"" + group.name + "\" is duplicated.");
        }
    }
...
void validate_boundary_edges(
    const RawMeshData& raw_mesh,
    const BoundaryIdSet& valid_boundary_ids)
{
    if (raw_mesh.boundary_edges.empty()) {
        throw_validation_error(
            "boundary_edges array is empty.");
    }
    ...
    for (std::size_t edge_index = 0; edge_index < raw_mesh.boundary_edges.size(); ++edge_index) {
        ...
        {
            throw_validation_error(
                "boundary edge " + std::to_string(edge_index) + " references a node outside the nodes array.");
        }
        if (node_a == node_b)
        {
            throw_validation_error(
                "boundary edge " + std::to_string(edge_index) + " references the same node twice.");
        }
        if (!valid_boundary_ids.contains(edge.boundary_id))
        {
            throw_validation_error(
                "boundary edge " + std::to_string(edge_index) + " references unknown boundary group ID "
                + std::to_string(edge.boundary_id) + ".");
        }
    ...
    {
        throw_validation_error(
            "the same boundary edge appears more than once.");
    }
    ...
    for (const BoundaryGroup& group : raw_mesh.boundary_groups)
    {
        ...
        {
            throw_validation_error(
                "boundary group \"" + group.name + "\" contains no boundary edge.");
        }
    }
}
} // namespace
