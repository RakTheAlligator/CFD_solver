...
[[noreturn]]
void throw_geometry_validation_error(
...
void validate_geometry_storage(
    ...
    GeometryStats& stats)
{
    ...
    for (Index cell_id = 0; cell_id < cell_count; ++cell_id)
    {
        ...
        if (!std::isfinite(area))
        {
            throw_geometry_validation_error(
                "cell " + std::to_string(cell_id) + " has non-finite area.");
        }

        if (!std::isfinite(center.x) || !std::isfinite(center.y))
        {
            throw_geometry_validation_error(
                "cell " + std::to_string(cell_id) + " has non-finite center coordinates.");
        }

        if (area <= 0.0)
        {
            throw_geometry_validation_error(
                "cell " + std::to_string(cell_id) + " has non-positive area.");
        }
        ...
        if (raw_mesh.cell_types[cell_id] == CellType::Triangle)
        {
            ...
            {
                throw_geometry_validation_error(
                    "cell " + std::to_string(cell_id) + " has invalid triangle quality.");
            }
    ...
    stats.cell_areas = {
        .minimum = area_stats.minimum,
        .maximum = area_stats.maximum,
        .mean = area_stats.sum / cell_count
    };

    stats.cell_sizes = {
        .minimum = size_stats.minimum,
        .maximum = size_stats.maximum,
        .mean = size_stats.sum / cell_count
    };

    if (triangle_count > 0)
    {
        stats.triangle_quality = {
            .minimum = triangle_quality_stats.minimum,
    ...
    GeometryStats& stats)
{
    ...
    for (Index face_id = 0; face_id < face_count; ++face_id)
    {
        ...
        {
            throw_geometry_validation_error(
                "face " + std::to_string(face_id) + " has non-finite length.");
        }

        if (!std::isfinite(center.x) || !std::isfinite(center.y))
        {
            throw_geometry_validation_error(
                "face " + std::to_string(face_id) + " has non-finite center coordinates.");
        }

        if (!std::isfinite(area_vector.x) || !std::isfinite(area_vector.y))
        {
            throw_geometry_validation_error(
                "face " + std::to_string(face_id) + " has non-finite area vector components.");
        }

        if (length <= 0.0)
        {
            throw_geometry_validation_error(
                "face " + std::to_string(face_id) + " has non-positive length.");
        }
        ...
        const double owner_orientation {
            area_vector.x * (center.x - owner_center.x)
        ...
        if (adjacency.neighbor != invalid_index)
        {
            ...
            const double owner_to_neighbor_orientation {
                area_vector.x * (neighbor_center.x - owner_center.x)
            ...
            {
                throw_geometry_validation_error(
                    "internal face " + std::to_string(face_id)
    ...
    stats.face_lengths = {
        .minimum = length_stats.minimum,
        .maximum = length_stats.maximum,
        .mean = length_stats.sum / face_count
    };
}
