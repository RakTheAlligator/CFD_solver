...
EigenConjugateGradientSolver::EigenConjugateGradientSolver(const ConjugateGradientOptions options) : options_(options)
{
    ...
    {
        throw std::invalid_argument("Conjugate-gradient relative tolerance must be finite and in (0, 1).");
    }
    if (options_.maximum_iterations == 0 ||
        options_.maximum_iterations > static_cast<Index>(std::numeric_limits<Eigen::Index>::max()))
    {
        throw std::invalid_argument("Conjugate-gradient maximum iterations must fit a positive Eigen index.");
    }
...
LinearSolveResult EigenConjugateGradientSolver::solve(const std::span<const double> rhs,
                                                      const std::span<double> solution)
{
    if (!matrix_is_prepared_)
    {
        throw std::logic_error("Eigen conjugate-gradient solve requires a prepared matrix.");
    }
    if (rhs.size() != matrix_size_ || solution.size() != matrix_size_)
    {
        throw std::invalid_argument("Eigen conjugate-gradient vectors must match the prepared matrix size.");
    }
    if (spans_overlap(rhs, solution))
    {
        throw std::invalid_argument("Eigen conjugate-gradient RHS and solution spans must not overlap.");
    }
    require_finite(rhs, "Eigen conjugate-gradient RHS must contain only finite values.");
    require_finite(solution, "Eigen conjugate-gradient initial guess must contain only finite values.");
    ...
    {
        throw std::runtime_error("Eigen conjugate-gradient solve failed numerically.");
    }
    require_finite(solution, "Eigen conjugate-gradient produced a non-finite solution.");
