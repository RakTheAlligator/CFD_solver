...
void validate_cell_storage(const RawMeshData& raw_mesh)
{
    ...
    if (raw_mesh.cell_node_offsets.size()
        != raw_mesh.cell_types.size() + 1) {

        throw_validation_error(
            "cell_node_offsets size must equal "
    ...
    for (std::size_t i = 1;
         i < raw_mesh.cell_node_offsets.size();
         ++i) {
        ...
        if (current > raw_mesh.cell_nodes.size()) {
            throw_validation_error(
                "cell_node_offsets contains an offset "
                "outside cell_nodes.");
        }
    }

    if (raw_mesh.cell_node_offsets.back()
...
void validate_cells(const RawMeshData& raw_mesh)
{
    for (Index cell_id = 0;
        ...
        const Index expected_count =
            expected_node_count(
                raw_mesh.cell_types[cell_id],
                cell_id);
        ...
        for (Index position = begin;
            ...
            if (node_id >= raw_mesh.nodes.size()) {
                throw_validation_error(
                    ...
                    + ", which is outside the nodes array.");
            }
            ...
            for (Index previous_position = begin;
                ...
                if (raw_mesh.cell_nodes[previous_position]
                    == node_id) {
    ...
    const RawMeshData& raw_mesh)
{
    ...
    for (const BoundaryGroup& group : raw_mesh.boundary_groups) {
        ...
        const bool id_inserted =
            boundary_ids.insert(group.id).second;
        ...
        const bool name_inserted =
            boundary_names.insert(group.name).second;
    ...
    const BoundaryIdSet& valid_boundary_ids)
{
    ...
    for (std::size_t edge_index = 0;
         edge_index < raw_mesh.boundary_edges.size();
         ++edge_index) {

        const BoundaryEdge& edge =
            raw_mesh.boundary_edges[edge_index];

        const Index node_a = edge.node_ids[0];
        const Index node_b = edge.node_ids[1];

        if (node_a >= raw_mesh.nodes.size()
            || node_b >= raw_mesh.nodes.size()) {
    ...
    std::sort(
        boundary_edge_keys.begin(),
        boundary_edge_keys.end());
...
void validate_raw_mesh(const RawMeshData& raw_mesh)
{
    ...
    const BoundaryIdSet boundary_ids =
        validate_boundary_groups(raw_mesh);
